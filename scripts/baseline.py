...
# =========================================================
# Path to the DeepSeek base model (the only model we'll use)
BASE_MODEL = "/mnt/st1/DeepSeekV3-2/DeepSeek-R1-Distill-Llama-8B" 

# We'll ignore this path since we're using the base model only
FINETUNED_MODEL = None

# Other paths - keep these the same
...
# Target vocabulary size - MODIFY IF NEEDED FOR YOUR MODEL
# The value below is specific to the DeepSeek model
# You may need to adjust this based on your new model's vocabulary size
...
def load_deepseek_model():
    """
    Simplified method to load just the base DeepSeek model without any fine-tuning.
    """
    print("Loading base DeepSeek model only (no fine-tuning)...")
    
    # Load tokenizer directly from base model
    ...
    # Add special tokens for consistency with the original script
    tokenizer.add_special_tokens({"pad_token": "<|pad|>"})
    special_tokens = ["ANSWER:"]
    tokenizer.add_special_tokens({"additional_special_tokens": special_tokens})
    tokenizer.padding_side = "right"
    print(f"Tokenizer loaded from base model with added tokens, vocab size: {len(tokenizer)}")
    ...
    # Resize token embeddings to match the tokenizer with added special tokens
    current_size = model.get_input_embeddings().weight.size(0)
    target_size = len(tokenizer)
    if current_size != target_size:
        print(f"Resizing model embeddings from {current_size} to match tokenizer size {target_size}")
        model.resize_token_embeddings(target_size)
    else:
        print(f"Model vocabulary size {current_size} already matches tokenizer size")
    
    print("Successfully loaded base model only (no fine-tuning applied)!")
    
    # Enable KV cache for efficient inference
    model.config.use_cache = True
    
    # Create pipeline with appropriate generation settings for base model
    pipe = pipeline(
        "text-generation",
        model=model,
        tokenizer=tokenizer,
    ...
    print(f"Pipeline created successfully, using base model only on device: {model.device}")
    return pipe
...
def main():
    # Update results directory to reflect base model only
    results_dir = "/mnt/st1/dir_ikeda/panasonic/HCQA/results/result_deepseek-base-only"
    os.makedirs(results_dir, exist_ok=True)
    
    print("\n=== USING BASE MODEL ONLY (NO FINE-TUNING) ===")
